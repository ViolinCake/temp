...
#     max_unavailable = 1
#   }
#   depends_on = [ aws_iam_role_policy_attachment.eks-demo-ng-AmazonEKS_CNI-policy,
...
resource "aws_eks_node_group" "eks-demo-node-group" {
  ...
  launch_template {
    name    = aws_launch_template.demo_node_launch_template.name
    version = aws_launch_template.demo_node_launch_template.latest_version
  }
  ...
  depends_on = [
    ...
    aws_launch_template.demo_node_launch_template, 
  ]
}