# --- 1. Data Source for Amazon Linux 2023 AMI ---
# We use a data source to always find the latest Amazon Linux 2023 AMI for the current region
# data "aws_ami" "amazon_linux" {
#   owners      = ["amazon"]
#   most_recent = true
#   filter {
#     name   = "name"
#     # Using the standard image for general use
...
resource "aws_security_group" "bastion_sg" {
  ...
  vpc_id      = aws_vpc.demo-eks-cluster-vpc.id # Replace with your VPC ID variable
...
resource "aws_instance" "bastion_host" {
  ...
  subnet_id                   = aws_subnet.public-subnet-1.id # Replace with your Public Subnet 1 ID variable
  ...
  user_data = <<-EOF
              ...
              # The 'aws eks update-kubeconfig' command is essential for
              # connecting to the EKS API server with the correct credentials.
              # This will run at the end of the script to ensure all tools are ready.
              
              # Placeholders - replace with your actual values (or use environment variables)
              # EKS_CLUSTER_NAME="your-eks-cluster-name"
              # EKS_REGION="your-aws-region"
